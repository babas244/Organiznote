...
if (isset($_SESSION['id']) && isset($_GET["sCategoriePere"]) && isset($_GET["idTopic"])) {
	...
	*/
	...
	$req = $bdd -> prepare('SELECT idNote, content, levelInTree, NbOfItems FROM notes WHERE idUser=:idUser AND idTopic=:idTopic AND idNote REGEXP :aTrouver AND idNote NOT LIKE :categoriepere AND isCategory=\'1\' ORDER BY rowOfNoteInCategory');
	$req -> execute(array(
	'idUser' => $_SESSION['id'],
	'idTopic' => $_GET["idTopic"], 
	'aTrouver' => '^'.$sCategoriePere.'(a[0-9]{2}$|[0-9]{2}$)',
