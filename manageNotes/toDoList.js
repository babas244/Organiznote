...
var aNbOfLabels = [5,3,3,3] // ‡ charger depuis la bdd
...
function displayToDoList (labelTitleRank, labelRank, isChecked) {
	...
	//alert (numberOfToDo);
	if (isChecked) {
		// tester d'abord s'il y a dÈj‡ un toDo de cette class dÈj‡ instanciÈ. vaudrait-il mieux avoir une variable globale qui dit si dÈj‡ chargÈ ou pas ? 
		if (numberOfToDo === 0) {
		...
		if (numberOfToDo !== 0) { // Áa peut arriver ?	
...
function stateToDoDone () {
	hideContextMenuToDo();
	if (confirm("√ätes-vous s√ªr de bien vouloir archiver comme faite la note :\n" + document.getElementById(toDoFocused).innerHTML) == true) {
